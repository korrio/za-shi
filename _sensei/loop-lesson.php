...
?>
    ...
    <?php if ( have_posts() ) { ?>
		<section id="main-course" class="course-container all-lessons">
            <section class="module-lessons">
        	    ...
        	    <?php while ( have_posts() ) { the_post();
                    ...
                    $html = '<article class="loop-lesson ' . esc_attr( join( ' ', get_post_class( array( 'lesson', 'course', 'post' ), $post_id ) ) ) . '">';

                        $html .= '<header>';

                            $html .= '<h2><a href="' . esc_url( get_permalink( $post_id ) ) . '" title="' . esc_attr( sprintf( __( 'Start %s', 'skillfully' ), get_the_title() ) ) . '">';

                            if( apply_filters( 'sensei_show_lesson_numbers', false ) ) {
                                $html .= '<span class="lesson-number">' . $lesson_count . '. </span>';
                            }

                            $html .= esc_html( sprintf( __( '%s', 'skillfully' ), get_the_title() ) ) . '</a></h2>';

                            $html .= '<p class="lesson-meta">';

                                if ( '' != $lesson_length ) { $html .= '<span class="lesson-length"><i class="fa fa-clock-o"></i> ' . $lesson_length . __( ' minutes', 'skillfully' ) . '</span>'; }
                                if ( isset( $woothemes_sensei->settings->settings[ 'lesson_author' ] ) && ( $woothemes_sensei->settings->settings[ 'lesson_author' ] ) ) {
                                    $html .= '<span class="lesson-author">' . get_avatar( get_the_author_meta('email'), '50' ) . '<a href="' . get_author_posts_url( absint( get_the_author_meta( 'ID' ) ) ) . '" title="' . esc_attr( $user_info->display_name ) . '">' . esc_html( $user_info->display_name ) . '</a></span>';
                                } // End If Statement
                                if ( '' != $lesson_complexity ) { $html .= '<span class="lesson-complexity"><i class="fa fa-signal"></i> ' . $lesson_complexity .'</span>'; }

                                if ( $single_lesson_complete ) {
                                    $html .= '<span class="lesson-status complete">' . apply_filters( 'sensei_complete_text', __( 'Complete', 'skillfully' ) ) .'</span>';
                                }
								elseif ( $user_lesson_status ) {
									$html .= '<span class="lesson-status in-progress"><i class="fa fa-spinner fa-pulse"></i> ' . apply_filters( 'sensei_in_progress_text', __( 'In Progress', 'skillfully' ) ) .'</span>';
								} // End If Statement
